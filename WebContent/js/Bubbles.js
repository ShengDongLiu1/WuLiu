...
+function()
{
	...
	function Bubble()
	{
		...
		oBox.setAttribute("class", "boxImg");
		styBox = oBox.style;
		...
		for(var i=0; i<4; i++)
		{
			...
			{
				if(_IE6_)
					sty.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src=images/lock/heart.png)";
				else
					sty.backgroundImage = "url(images/lock/heart.png)";
				break;
			}
			...
			if(STD)
			{
				sty.backgroundImage = "url(images/lock/ch" + i + ".png)";
				arrFlt[i] = sty;
			}
			else
			{
				sty.filter = "alpha progid:DXImageTransform.Microsoft.AlphaImageLoader(src=../images/lock/ch" + i + ".png)";
